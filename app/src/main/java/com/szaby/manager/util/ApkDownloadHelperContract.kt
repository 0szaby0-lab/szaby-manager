...
package com.szaby.manager.util
...
object ApkDownloadHelperContract {
    const val ACTION_DOWNLOAD_ORIGINAL_APK = "com.szaby.manager.action.DOWNLOAD_ORIGINAL_APK"
    ...
    const val EXTRA_PROTOCOL_VERSION = "com.szaby.manager.extra.PROTOCOL_VERSION"
     ...
     */
    const val EXTRA_CALLER_PACKAGE = "com.szaby.manager.extra.CALLER_PACKAGE"

    const val EXTRA_PACKAGE_NAME = "com.szaby.manager.extra.PACKAGE_NAME"
    const val EXTRA_APP_NAME = "com.szaby.manager.extra.APP_NAME"

    /** Version the user asked for. Absent when any compatible version will do. */
    const val EXTRA_VERSION_NAME = "com.szaby.manager.extra.VERSION_NAME"

    /** Build codes of [EXTRA_VERSION_NAME] as a `long[]`. Empty when the bundle declares none. */
    const val EXTRA_VERSION_CODES = "com.szaby.manager.extra.VERSION_CODES"

    /** Other version names Morphe can patch, for when the requested one is no longer available. */
    const val EXTRA_COMPATIBLE_VERSION_NAMES = "com.szaby.manager.extra.COMPATIBLE_VERSION_NAMES"

    /** Device ABIs in preference order, mirroring [Build.SUPPORTED_ABIS]. */
    const val EXTRA_SUPPORTED_ABIS = "com.szaby.manager.extra.SUPPORTED_ABIS"

    /** Archive format the patch bundle expects, one of the `FILE_TYPE_` constants. */
    const val EXTRA_FILE_TYPE = "com.szaby.manager.extra.FILE_TYPE"

    /** Whether a split archive is acceptable. False when the bundle requires a plain APK. */
    const val EXTRA_ALLOW_SPLIT_ARCHIVE = "com.szaby.manager.extra.ALLOW_SPLIT_ARCHIVE"
     ...
     */
    const val EXTRA_STOCK_INSTALL_REQUIRED = "com.szaby.manager.extra.STOCK_INSTALL_REQUIRED"

    /** Web search Morphe would have opened, so a helper can fall back to it. */
    const val EXTRA_FALLBACK_WEB_URL = "com.szaby.manager.extra.FALLBACK_WEB_URL"
     ...
     */
    const val EXTRA_RESULT_USE_INSTALLED_APP = "com.szaby.manager.extra.RESULT_USE_INSTALLED_APP"

    /** The package an installed-app result refers to. Must be the one Morphe asked for. */
    const val EXTRA_RESULT_PACKAGE_NAME = "com.szaby.manager.extra.RESULT_PACKAGE_NAME"
